...
The following packages are required for Secure Boot to work. You can install
them all at once by using the `proxmox-secure-boot-support` meta-package.
...
Only GRUB is supported as bootloader out of the box, since other bootloaders are
currently not eligible for secure boot code-signing.

Any new installation of {pmg} will automatically have all of the above packages
included.
